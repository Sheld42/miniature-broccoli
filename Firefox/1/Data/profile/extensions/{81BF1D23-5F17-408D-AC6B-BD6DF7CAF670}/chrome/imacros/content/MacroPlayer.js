...
iMacros.player = (function() {
    ...
    Components.utils.import("resource://gre/modules/Downloads.jsm", scope);

    var __loginf = scope.__loginf;
    var imns = scope.imns;
    var Rijndael = scope.Rijndael;
    var ctypes = scope.ctypes;
    var Downloads = scope.Downloads;
        ...
        if (!target || !rect.width || !rect.height) {
            ...
            this.retry(function() {
                ...
                if (!target)
                    ...
                    throw new RuntimeError(
                        "Element "+target.tagName+" is not visible"
                    );
    ...
    MacroPlayer.prototype.onImrMessage = function(evt) {
        ...
        if (msg.type == "message") {
            ...
            this.showErrorAndStop(
                new RuntimeError(msg.error, this.ignoreErrors)
            );
            return;
        } 
        ...
        if (msg.rv == TM_STATUS_MATCH_FOUND_OK) {
            ...
            this.showErrorAndStop(
                new RuntimeError("Can not open image file "+msg.template)
            );
        } else {
            this.showErrorAndStop(
                new RuntimeError("Image search error "+msg.rv)
            );
        }
    };
        ...
        if (!this.imr_worker) {
            ...
            if (!libpath) {
                if (this.ignoreErrors) {
                    ...
                    this.showErrorAndStop(new RuntimeError(msg_no_free_beer));
                    return;
                }
            }
    ...
    MacroPlayer.prototype.calculateFileHash = function (file, alg) {
        ...
        if (alg == "MD5") {
            ...
            throw new RuntimeError("Unknown Hash algorithm "+alg);
        }
        ...
        var s = [tohex(hash.charCodeAt(i)) for (i in hash)].join("");
    ...
    MacroPlayer.prototype.saveURI = function(uri, file, wait) {
        var wbp = imns.Cc[
        ...
        try {
            ...
            var pcontext = this.currentWindow.
                QueryInterface(imns.Ci.nsIInterfaceRequestor).
                getInterface(imns.Ci.nsIWebNavigation).
                QueryInterface(imns.Ci.nsILoadContext);
        } catch(e) {
            Components.utils.reportError(e);
        }
        ...
        wbp.progressListener = {
            ...
            onStateChange: function(web_progress, request, state, status) {
                if (!(state & STATE_STOP))
                    return;
                ...
                try {
                    var check = mplayer.calculateFileHash(
                        file, mplayer.downloadCheckAlg
                    );
                    if (check != mplayer.downloadChecksum) {
                        
                        mplayer.showErrorAndStop(
                            new RuntimeError(
                            "Checksum of downloaded file "+check+
                            " doesn't match specified", -934),
                            mplayer.ignoreErrors
                        );
                        mplayer.downloadChecksum = "";
                        mplayer.downloadCheckAlg = "";
                    }
                } catch (e) {
                    mplayer.showErrorAndStop(e, mplayer.ignoreErrors);
                }
    ...
    MacroPlayer.prototype.saveTargetAs = function(element) {
        ...
        this.saveURI(uri, file, this.shouldWaitDownload);
    ...
    MacroPlayer.prototype.saveAsScreenshot = function(element) {
        ...
        try {
            ...
            var pcontext = win.QueryInterface(imns.Ci.nsIInterfaceRequestor).
                getInterface(imns.Ci.nsIWebNavigation).
                QueryInterface(imns.Ci.nsILoadContext);
        } catch(e) {
            Components.utils.reportError(e);
    ...
    MacroPlayer.prototype.savePageAsImage = function(win, filename, folder, type, callback) {
        ...
        var wbp = imns.Cc['@mozilla.org/embedding/browser/nsWebBrowserPersist;1'];
        wbp = wbp.createInstance(imns.Ci.nsIWebBrowserPersist);
        var flags = wbp.PERSIST_FLAGS_REPLACE_EXISTING_FILES |
            wbp.PERSIST_FLAGS_AUTODETECT_APPLY_CONVERSION;
        wbp.persistFlags = flags;
        ...
        if (callback) {
            ...
            wbp.progressListener = {
                onProgressChange: function(
                    progress, request, cur_progress, max_progress,
    ...
    MacroPlayer.prototype.ActionTable["screenshot"] = function (cmd) {
        ...
        try {
            var f = folder ? imns.FIO.openNode(folder) :
    ...
    MacroPlayer.prototype.globalTimer = {
        ...
        setMacroTimeout: function(x) {
            ...
            this.macroTimeout = setTimeout( function () {
                mplayer.showErrorAndStop(
                    new RuntimeError(
                        "Max. macro runtime was reached. Macro stopped.",
                            -803)
                );
            }, Math.round(x*1000));
        },
        ...
        handleSelectElement: function(element, content_value) {
            ...
            if(m = content_value.match(re)) {
                ...
                if (!/^\d+$/.test(content_value))
                    throw new RuntimeError("Wrong format of CONTENT value");
            }
    ...
    MacroPlayer.prototype.compileExpressions = function () {
        for (var x in this.RegExpTable) {
            try {
                ...
                __loginf("failed on compiling regexp "+x);
                throw e;
            }
        }
    };
    ...
    MacroPlayer.prototype.onStateChange = function(progress, req, flag, stat) {
        ...
        if(flag & STATE_START) {
            ...
            this.networkError = !(Components.isSuccessCode(stat) ||
                                  stat == Cr.NS_IMAGELIB_ERROR_LOAD_ABORTED ||
                                  stat == Cr.NS_ERROR_PARSED_DATA_CACHED ||
                                  stat == Cr.NS_BINDING_ABORTED);
    ...
    MacroPlayer.prototype.saveStopwatchResults = function() {
        ...
        var now = new Date();
        var d = imns.formatDate("yyyy/mm/dd", now);
        var t = imns.formatDate("hh:nn", now);
        var newline = imns.is_windows() ? "\r\n" : "\n";
        var s = "\"Date: "+d+"  Time: "+t+
        ...
        for (var i = 0; i < this.stopwatchResults.length; i++) {
            ...
            var timestamp = imns.formatDate("yyyy/mm/dd,hh:nn:ss",
                                            r.timestamp);
        ...
        if (this.stopwatchFile) {
            ...
            var filename = /^(.+)\.iim$/.test(iMacros.currentMacro.name) ?
