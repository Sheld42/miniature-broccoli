...
function checkSIActive() {
    if (!imns.is_windows())
        return;
    var sibox = document.getElementById("imacros-si-info");
    var sitext = document.getElementById("imacros-si-text");
    const href = "http://avtoskript.ru";
    sibox.collapsed = false;
    sitext.value = "Программа Avtoskript";
    sitext.href = href+"";
    ...
    try {
        ...
        if (wrk.hasChild("iMacros")) {
            ...
            if (subkey.hasValue("PathBasic")) { 
                sitext.value = "Scripting Active (Trial)";
                sitext.href = href+"siactivetr";
                if (subkey.hasValue("InterfaceType") &&
                    /^fx/.test(subkey.readStringValue("InterfaceType"))) {
                    sitext.value = "Scripting Active";
                    sitext.href = href+"siactive";
                }
            }
            subkey.close();
        }
