...
iMacros.conv_recorder = (function() {
    ...
    MacroRecorder.prototype.handleDownloadDialog = function (data) {
        var rec = "ONDOWNLOAD FOLDER=*"+
            ...
            " WAIT=YES";
        var lastrec = this.actions.pop();
        if (/^TAB T=\d+/.test(lastrec) && this.actions.length > 2) {
            
            
            var prev2 = this.actions[this.actions.length-1];
            var prev3 = this.actions[this.actions.length-2];
            if (prev2 == "TAB CLOSE" && /^TAB T=\d+/.test(prev3)) {
                this.actions.pop();
                this.actions.pop();
                iMacros.panel.removeLastLine();
                iMacros.panel.removeLastLine();
                iMacros.panel.removeLastLine();
                lastrec = this.actions.pop();
            }
        }
        iMacros.panel.removeLastLine();
        this.recordAction(rec);
