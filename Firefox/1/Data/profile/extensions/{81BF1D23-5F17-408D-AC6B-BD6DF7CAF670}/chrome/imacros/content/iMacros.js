...
var iMacros = ( function() {
    ...
    let scope = {};
    Cu.import("resource://imacros/utils.js", scope);
    let {SOAPClient} = Cu.import("resource://imacros/SOAPClient.js");

    let __loginf = scope.__loginf;
    let imns = scope.imns;
    ...
    obj.playJSFile = function(file) {
        try {
            ...
            let (data) {
                ...
                if (!data.length) 
                    throw new Error("File "+file.path+
                                    " is empty or can not be read");
                this.jssrc = data;
            }
    ...
    obj.observe = function (subject, topic, data) {
        if (topic == "quit-application-granted") {
            ...
            if (subject != window)
                return;
            setTimeout(function () { iMacros.playURLMacro(data) }, 0);
        }
